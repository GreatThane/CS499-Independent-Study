...
export async function downvote(post_id, author) {
    if (!VOTE_POST_SQL) VOTE_POST_SQL = await fs.readFile('src/sql_scripts/vote_post.sql', 'utf-8');
    return await query(VOTE_POST_SQL, [post_id, author, -1]);
}
...
export default async function handler(req, res) {
    if (req.method === "POST") {
        ...
        } else res.status(200).json(await downvote(req.query.post_id, req.body.author));
