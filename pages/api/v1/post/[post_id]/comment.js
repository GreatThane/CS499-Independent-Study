...
export default async function handler(req, res) {
    ...
    if (req.method !== "POST") {
        res.status(405).json({status: "Error: Must be a post request."});
        return;
    }
    if (!req.body.author) {
        ...
        res.status(400).json({status: "Error: 'post_id' parameter required."});
    } else res.status(200).json(await query(MAKE_COMMENT_SQL, [req.body.content, req.body.author, req.body.post_id]));
}
