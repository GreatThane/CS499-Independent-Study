...
export async function downvote(answer_id, author) {
    if (!VOTE_POST_SQL) VOTE_POST_SQL = await fs.readFile('src/sql_scripts/vote_post.sql', 'utf-8');
    return await query(VOTE_POST_SQL, [answer_id, author, -1]);
}
...
export default async function handler(req, res) {
    if (req.method === "POST") {
        ...
        } else res.status(200).json(await downvote(req.query.answer_id, req.body.author));
